...
The artefacts in this repository cite a working apparatus: numbered threads (T1 to T10), instruments (I1, I2), verification tasks (V1 to V6) and validation checkpoints. That apparatus lives in a private cross-framework synthesis file, the corpus's working record, which is deliberately not published: it is a living document whose public snapshot would go stale, and it carries developing findings at confidence levels chosen to stay internal. This page is the public key to the references, so that a citation like "synthesis T2" resolves to a question rather than a dead end.
...
These artefacts are produced in structured collaboration with Claude (Anthropic): the analytical judgement, rulings and verdicts are the author's; drafting, verification mechanics and consistency checking are run with the model under the discipline described above. That division is treated as method, not caveat.
